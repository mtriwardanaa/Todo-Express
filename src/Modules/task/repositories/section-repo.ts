...
const getOneSection = async (id: string): Promise<SectionRes | null> => {
...
const createSection = async (data: SectionReq): Promise<SectionRes> => {
  data.order = ((await getSection(true)) as unknown as number) + 1;
...
const updateSection = async (
  ...
  Object.entries(data).forEach(async ([key, value], index) => {
    if (key === 'order') {
      ...
      const getOne = await getOneSection(id);
  ...
  if (reorder === 1) {
    await reorderSection(sort);
  }

  return section;
};

const deleteSection = async (id: string): Promise<DeleteResult> => {
...
const reorderSection = async (
  ...
  const section: any = await getSection(false, sort);
