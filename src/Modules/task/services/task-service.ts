...
class TaskService {
    ...
    getTask = async (projectId: string) => {
        ...
        const getTaskWithSection = await SectionRepo.getSectionWithTask(
            projectId
        )
        return {
            status: true,
    ...
    createTask = async (dataReq: Task, projectId: string) => {
        ...
        if (!checkNull(dataReq.section_id)) {
            const checkSection = await SectionRepo.getOneSection(
                dataReq.section_id
            )
            if (checkSection) {
        ...
        const order = ((countTask as unknown as number) +
                ...
                if (newData.new_parent_id != '-') {
                    ...
                    if (!checkNull(checkNewParent.parent_id)) {
                        return {
                            status: false,
                            message:
                                'parent already has parent id, not available now',
                        }
                    }
